...
class Pic4rlTurtleBot3(Pic4rlGymGazEnv):
	...
	def get_goal(self):
		random_amplitude = np.min((self.episode+1, 99) )/100 * np.pi
		distance = np.random.uniform(0.5, 2, 1)	
		angle = np.random.uniform(-random_amplitude, random_amplitude, 1) + np.pi/2
